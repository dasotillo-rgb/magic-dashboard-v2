...
def calculate_supertrend(df, period, multiplier):
    # (The Supertrend calculation logic remains the same)
...
# --- Calibration Engine v3 ---
def run_dual_calibration(historical_data):
    print("\nStarting DUAL calibration for BTC & SOL Flip Levels...")
    periods_to_test = range(5, 25)
    multipliers_to_test = [i * 0.1 for i in range(10, 61)]
        ...
        for multiplier in multipliers_to_test:
            ...
            for ticker, target_level in TARGETS.items():
                ...
                if latest_trend == -1:
                    ...
                    # Use percentage error for fair comparison
                    error = abs(calculated_flip_level - target_level) / target_level
                    combined_error += error
                else:
                    # If trend is not bearish, it's a bad fit, assign high error
                    combined_error += 999 
...
if __name__ == "__main__":
    ...
    if len(all_data) == len(tickers_to_test):
        run_dual_calibration(all_data)
    else:
        print("Could not fetch data for all required tickers. Calibration aborted.")

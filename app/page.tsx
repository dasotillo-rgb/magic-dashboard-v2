  ...
  return (
    <div className="container">
      <style jsx global>{`
        ...
        .sidebar { width: 260px; border-right: 1px solid var(--border); padding: 40px 24px; display: flex; flex-direction: column; background: rgba(0,0,0,0.2); backdrop-filter: blur(10px); }
        ...
        .nav-item { padding: 14px 18px; border-radius: 12px; cursor: pointer; transition: all 0.3s; color: #666; font-weight: 500; font-size: 14px; display: flex; align-items: center; gap: 12px; }
        ...
        .dashboard-grid { display: grid; grid-template-columns: repeat(12, 1fr); grid-auto-rows: minmax(160px, auto); gap: 24px; }
        .widget { background: var(--glass); border: 1px solid var(--border); border-radius: 28px; padding: 30px; backdrop-filter: blur(40px); position: relative; transition: transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
        ...
        .btn-notion { background: white; color: black; border: none; padding: 10px 20px; border-radius: 99px; font-weight: 700; font-size: 12px; display: flex; align-items: center; gap: 8px; cursor: pointer; transition: 0.3s; }
      ...
      <aside className="sidebar">
        <div className="logo"><Icons.Zap /> Ape Intelligence</div>
      ...
      <main className="viewport">
        <header className="top-bar">
          <div><h1 style={{ margin: 0, fontSize: '28px', fontWeight: 800 }}>{activeTab}</h1><p className="text-mute">System operational.</p></div>
          <button className="btn-notion" onClick={handleSyncNotion}><Icons.Notion /> {syncStatus === 'Idle' ? 'SYNC TO NOTION' : syncStatus.toUpperCase()}</button>
        </header>
        <div className="dashboard-grid">
          <div className="widget" style={{ gridColumn: 'span 3' }}>
            <span className="widget-title">Local Environment</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}><Icons.Cloud /><div className="val-md">17°C</div></div>
            <div style={{ marginTop: '20px', fontSize: '12px' }}>{time.toLocaleTimeString()}</div>
          </div>
          <div className="widget" style={{ gridColumn: 'span 5' }}>
            <span className="widget-title">Trend Signal</span>
            <div className="val-lg" style={{ color: 'var(--green)' }}>STRONG BUY</div>
            <div className="progress-container"><div className="progress-bar" style={{ width: '92%', background: 'var(--green)' }}></div></div>
          </div>
          <div className="widget" style={{ gridColumn: 'span 4' }}>
            <span className="widget-title">Daily Ventures</span>
            <div style={{ fontSize: '13px' }}>#1 Automatización Logística</div>
          </div>
          <div className="widget" style={{ gridColumn: 'span 12' }}>
            <span className="widget-title">Console</span>
            <div style={{ fontFamily: 'monospace', color: '#10b981' }}>&gt; ApeOS V2 Ready.</div>
          </div>
        </div>
      </main>
    </div>
